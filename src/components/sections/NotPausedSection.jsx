...
export function NotPausedSection() {
  ...
  return (
    <section id="notpaused" ref={ref} className="bg-[#F8FAFC] border-b border-[#E2E8F0]/60 scroll-mt-20">

      {/* ── HERO BANNER ── */}
      <div className="relative overflow-hidden bg-gradient-to-br from-[#3B0764] via-[#5B21B6] to-[#4C1D95] py-28 px-6 md:px-12 lg:px-20">
      ...
      <div className="container mx-auto px-6 md:px-12 lg:px-20 py-20 space-y-20">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {services.map((s, i) => (
              <div key={i} className="reveal shimmer-card bg-white/80 border border-[#E2E8F0]/60 p-8 rounded-3xl shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 space-y-5" style={{ transitionDelay: `${i * 100}ms` }}>
                ...
                <h4 className="font-serif font-bold text-[#0F172A] text-lg">{s.title}</h4>
                <ul className="space-y-2">
                  {s.items.map((item, j) => (
                    <li key={j} className="flex items-start gap-2 text-[#475569] text-sm font-light">
                      <span className="text-[#5B21B6] mt-0.5 flex-shrink-0">✦</span>
                      {item}
                    </li>
                  ))}
                </ul>
        ...
        {/* Closing Quote */}
        <div className="reveal relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#3B0764] via-[#5B21B6] to-[#4C1D95] p-10 lg:p-14 text-center shadow-2xl">

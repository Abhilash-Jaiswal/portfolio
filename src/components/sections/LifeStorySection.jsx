  ...
  return (
    <button
      ...
      className={`flex items-center gap-2 px-4 py-2 rounded-full text-[10px] font-bold tracking-widest uppercase transition-all duration-300 cursor-pointer whitespace-nowrap
        ${active
          ? "bg-[#A054A6] text-white shadow-md shadow-[#A054A6]/30 scale-105"
          : "bg-white/70 text-stone-600 border border-stone-200/60 hover:bg-white hover:text-[#A054A6]"}`}
    >
...
function StoryCard({ heading, body, delay = 0 }) {
  ...
  return (
    <div
      ref={ref}
      className="reveal shimmer-card bg-white/85 border border-stone-200/60 p-7 rounded-3xl shadow-sm hover:shadow-lg hover:border-[#A054A6]/30 hover:-translate-y-1 transition-all duration-400 group"
      style={{ transitionDelay: `${delay}ms` }}
    >
      <h4 className="font-serif font-bold text-stone-900 text-base mb-3 group-hover:text-[#A054A6] transition-colors">{heading}</h4>
      <p className="text-stone-600 text-sm font-light leading-relaxed text-justify">{body}</p>
    </div>
  );
}
...
export function LifeStorySection() {
  ...
  return (
    <section id="life-story" className="bg-[#F7ECF2] border-b border-stone-200/60 scroll-mt-20">

      {/* ── HERO BANNER ────────────────────────────── */}
      <div className="relative overflow-hidden bg-gradient-to-br from-[#A054A6] via-[#8b3fa6] to-[#6b2fa0] py-28 px-6 md:px-12 lg:px-20">
        {/* Animated blobs */}
      ...
      {/* ── STICKY CHAPTER NAV ─────────────────────── */}
      <div className="sticky top-20 z-40 bg-[#F7ECF2]/95 backdrop-blur-md border-b border-stone-200/60 shadow-sm">
      ...
      <div className="container mx-auto px-6 md:px-12 lg:px-20 pt-20 pb-8">
        <div className="reveal max-w-4xl mx-auto bg-white/60 border-l-4 border-[#A054A6] rounded-r-3xl px-8 py-6 shadow-sm">
          <p className="font-serif text-lg lg:text-xl italic text-stone-800 leading-relaxed">
      ...
      <div className="container mx-auto px-6 md:px-12 lg:px-20 py-12 space-y-32">
        {CHAPTERS.map((chap, ci) => (
          ...
          >
            ...
            <div className="reveal flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <h2 className="font-serif text-2xl lg:text-3xl text-stone-900 font-bold tracking-tight leading-tight">{chap.title}</h2>
                  <p className="text-stone-500 text-xs font-medium tracking-widest uppercase mt-1">{chap.sub}</p>
                </div>
              </div>
              ...
              <div className="hidden md:flex items-center gap-1">
                {CHAPTERS.map((_, i) => (
                  <div
                    ...
                    className={`h-1.5 rounded-full cursor-pointer transition-all duration-500 ${CHAPTERS[i].num === chap.num ? "w-8 bg-[#A054A6]" : "w-2 bg-stone-300 hover:bg-[#A054A6]/50"}`}
                  />
                ))}
              </div>
            </div>
            ...
            >
              <p className="text-stone-700 text-base lg:text-lg font-light leading-relaxed">{chap.intro}</p>
            </div>
            ...
            <div className="reveal max-w-3xl">
              <div className="flex items-start gap-4 bg-gradient-to-r from-stone-50 to-white/60 border border-stone-200/60 rounded-2xl px-6 py-5 shadow-sm">
                <span className="text-2xl flex-shrink-0 mt-0.5">✦</span>
                <p className="text-stone-700 text-sm font-medium leading-relaxed italic">{chap.closing}</p>
              </div>
            </div>
      ...
      <div className="container mx-auto px-6 md:px-12 lg:px-20 pb-20">
        <div className="reveal text-center mb-12">
          <div className="inline-block border-t-[3px] border-[#A054A6] pt-2 mb-4">
            <span className="text-[#A054A6] font-bold text-sm tracking-[0.2em] uppercase">Three Convictions That Organise an Entire Life</span>
          </div>
          <h3 className="font-serif text-2xl lg:text-3xl text-stone-900 font-bold">A Philosophy Lived Out Loud</h3>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-20">
          {[
            ...
            <div
              key={i}
              className="reveal shimmer-card bg-white/80 border border-stone-200/60 p-8 rounded-3xl shadow-sm hover:shadow-xl hover:border-[#A054A6]/30 hover:-translate-y-1 transition-all duration-300"
            ...
            >
              ...
              <h4 className="font-serif font-bold text-stone-900 text-lg mb-3">{p.title}</h4>
              <p className="text-stone-600 text-sm font-light leading-relaxed">{p.body}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="mx-6 md:mx-12 lg:mx-20 mb-20 reveal">
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#A054A6] via-[#8b3fa6] to-[#6b2fa0] p-12 lg:p-16 text-center shadow-2xl">
          {/* decorative blobs */}
